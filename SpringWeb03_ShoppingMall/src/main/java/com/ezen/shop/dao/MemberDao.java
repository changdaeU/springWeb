...
public class MemberDao {
	...
	// 5. 추가된 메서드에 Dto 객체를 생성하고, 각 멤버변수를 rs.getInt or rs.getString 등을 이용하여 set 합니다
	// 6. 다 채워진 Dto 객체를 리턴합니다.
	// 7. 마지막 괄호 앞에(,)로 구분하여 ?에 들어갈 인수를 차례로 써줍니다.
	// 8. List가 아닌 단일 Dto가 리턴되어야 한다면 list.get(0)을 리턴합니다. 단, 리턴값이 null일 경우에는 if로 처리합니다.
	...
	public int confirmID(String id) {
		...
		if(list.size()==0) result = 1; // 사용가능 , 검색한 아이디 없음
		else result = -1 ; // 사용 불가능, 검색한 아이디가 존재함
		return result;
	}
